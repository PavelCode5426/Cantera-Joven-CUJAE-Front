  ...
  <el-dialog v-model="dialogVisible" title="Crear nuevo APIKEY" :closed="clearForm()" :show-close="false">
    <el-form>
      <el-form-item label="Nombre del Sistema">
        <el-input v-model="form.name" data-test="input-api-token-name" @blur="v.name.$touch()" />
        <error-help-block :items="v.name.$errors" />
      </el-form-item>
      <el-form-item label="Fecha de Expiracion">
        <el-date-picker v-model="form.expired_at" data-test="calendar-api-token-expiration_at" />
      </el-form-item>
    </el-form>
    <template #footer>
      <div class="modal-footer">
        <button class="btn btn-info" :class="{ disabled: !form.name.length }" data-test="btn-submit-api-key-form" @click="submitForm()">
          <loading v-if="isLocalLoading" /> Aceptar
        </button>
        <button class="btn btn-default" data-test="btn-cancel-api-key-form" @click="closeDialog()">
          Cerrar
        </button>
      </div>
    </template>
