...
const etapa = computed(() => {
  return searchEtapa(props.actividad.etapa)
})
const can_manage_actividad = computed(() => {
  return planStore.value.estado === EstadoPlanFormacion.aprobado && etapa.value.evaluacion === null
})
const can_manage_execution = computed(() => {
  return (is_joven() || is_tutor()) && can_manage_actividad.value
})
</script>
