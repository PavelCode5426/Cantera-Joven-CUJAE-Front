...
async function newCommentHandler(comentario: ComentarioModel) {
  try {
    const response = await comentarioService.add_comentario_to_plan_individual(props.plan?.id, comentario.texto)
...
<template>
  ...
  <comentario-form v-if="plan?.estado === EstadoPlanFormacion.pendiente" @new="newCommentHandler" />
</template>
