...
import { ref } from 'vue'
import { get_current_area_id } from '../../../../helpers/utils'
...
const area = get_current_area_id()
...
async function loadData(filter: Filter) {
  try {
    const response = await FIndivServices.list_planes_formacion_from_area(area, filter)
...
<template>
  <h3>Planes de Formacion Individual del Area</h3>
