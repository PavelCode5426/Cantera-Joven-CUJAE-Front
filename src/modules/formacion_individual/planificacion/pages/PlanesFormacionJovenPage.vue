...
const joven_id = route.params.id
...
async function loadData(filter: Filter) {
  try {
    const response = await FIndivServices.list_planes_formacion_from_joven(joven_id, filter)
...
<template>
  ...
  <plan-formacion-filter-form v-model:filter="filter" :searchable="false" @submit="filterFormHandler" />
